...
void IRBosch144AC::send(const uint16_t repeat) {
  if (ledFlag) { // Flip display state 96bit message
    _irsend.sendBosch144(kBosch144FlipLed, sizeof(kBosch144FlipLed), repeat);
    ledFlag = false;
  } else {
    ...
    /* Serial.print("tOffFlag: ");
    Serial.println(tOffFlag);
    Serial.print("raw:");
    for (uint8_t i = 0; i < sizeof(_.raw); i++) {
      Serial.print(_.raw[i], BIN);
    }
    Serial.println(); */
  }
  }
...
/// Get a copy of the internal state as a valid code for this protocol. Process timer.
/// @return A valid code for this protocol based on the current internal state.
unsigned char* IRBosch144AC::getRaw(void) {
  switch (tOffFlag){  //timer pre process
    case 0:
    _.TimerOffS1 = _.TimerOffS2 = 0b11111;
    break;
    case 1:
    _.TimerOffS1 = _.TimerOffS2 = 0b00001;
    break;
    case 2:
    _.TimerOffS1 = _.TimerOffS2 = 0b00011;
    break;
    case 3:
    _.TimerOffS1 = _.TimerOffS2 = 0b00111;
    break;
    default:
    break;
  }
  ...
  if (tOffFlag != 0 ){ //timer post process
    _.InnvertS1_3 = _.InnvertS2_3 = 0b01111111;
  }
...
/// Prende o apaga el display, invierte el estado.
/// @param[in] led true, cambia estado de display. false, no hay cambio.
void IRBosch144AC::flipLed(const bool led) {
  ledFlag = led;
}

/// Set timer to turn off AC.
/// @param[in] toff, 0 off, 1 0.5h, 2 1h, 3 2h.
void IRBosch144AC::setTimerOff(const uint8_t toff) {
  tOffFlag = toff;
}

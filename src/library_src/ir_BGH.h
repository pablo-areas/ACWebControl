...
//   Brand: BGH,  Remote control: RG10A(G2S)BGEF
...
// cambia el estado del display 96bit-message
const uint8_t kBosch144FlipLed[] = {0xB9, 0x46, 0xF5, 0x0A, 0x09, 0xF6,
                                    0xB9, 0x46, 0xF5, 0x0A, 0x09, 0xF6};
...
union Bosch144Protocol {
  ...
  struct {
    ...
    uint8_t TimerOffS1 :5;   // not used (without timer use)     #              TimerOffS1 :5;
    ...
    uint8_t TimerOffS2 :5;   // not used (without timer use)     #                TimerOffS2 :5;
...
class IRBosch144AC {
 public:
  ...
  void flipLed(const bool led);
  void setTimerOff(const uint8_t toff);
  uint8_t* getRaw(void);
 ...
 private:
  ...
  bool powerFlag;
  bool ledFlag;
  uint8_t tOffFlag; //0 no timer off, 1 0.5h TimerOff 00001, 2 1h TimerOff 00011, 3 2h TimerOff 00111
